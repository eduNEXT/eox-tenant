...
upgrade: ## update the requirements/*.txt files with the latest packages satisfying requirements/*.in
	...
	grep -e "^django==" -e "^djangorestframework==" -e "^edx-opaque-keys" requirements/test.txt > requirements/django.txt
	sed '/^[dD]jango==/d;/^djangorestframework==/d;/^edx-opaque-keys/d' requirements/test.txt > requirements/test.tmp
