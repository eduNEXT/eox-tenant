...
class EoxTenantAPIPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        To grant access, checks if the requesting user:
            1) it's a staff user
            2) it's allowed to perform operations on the site (check user signUp sources)
        ...
        user = request.user
        user_sources = user.usersignupsource_set.all()

        if user.is_staff:
            return True

        if not self.is_authorized_source_for_user(host, user_sources):
            return False
        ...
        try:
            client_url_allowed = host in request.auth.client.url
        except Exception:  # pylint: disable=broad-except
    ...
    @staticmethod
    def is_authorized_source_for_user(request_host, user_signup_sources):
        """
        Returns True if the current user has a valid signUp source to make requests
        to the current site.
        """
        authorized_sources = getattr(settings, 'EDNX_ACCOUNT_REGISTRATION_SOURCES', [request_host])

        for source in user_signup_sources:
            if any(re.match('{0}$'.format(pattern), source.site, re.IGNORECASE) for pattern in authorized_sources):
                return True
